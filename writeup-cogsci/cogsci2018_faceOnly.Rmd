...
dGLM_faces <- d %>%
  ...
  summarize(countDetected_Faces=sum(faceMT==TRUE), countTotal_Frames = sum(faceMT==FALSE), age.at.test=age.at.test[1]) 

dGLM_wrists <- d %>%
  group_by(subid, posture, orientation) %>%
  summarize(countDetected_Wrists=sum(wristOP==TRUE), countTotal_Frames = sum(wristOP==FALSE),   age.at.test=age.at.test[1]) 
...
y_faces=cbind(dGLM_faces$countDetected_Faces, dGLM_faces$countTotal_Frames)  
y_wrists=cbind(dGLM_wrists$countDetected_Wrists, dGLM_wrists$countTotal_Frames) 

# run models
glm_faces<-glm(y_faces ~ dGLM_faces$age.at.test * dGLM_faces$posture * dGLM_faces$orientation,family=binomial(logit))
xtable(summary(glm_faces))
...
faceGLMOut=xtable(summary(glm_faces))
rownames(faceGLMOut)<-cleanedNames

wristGLMOut=xtable(summary(glm_wrists))
rownames(wristGLMOut)<-cleanedNames

```

```{r results="asis"}
print(faceGLMOut, type="latex", comment = F, table.placement = "H")
print(wristGLMOut, type="latex", comment = F, table.placement = "H")
```
...
<!-- # ## Then try a run mixed effet model -->
...
<!-- #No convergence issues with open pose detectors -->
<!-- fullModelminsInt_Faces_OP <- glmer(faceOP ~ scale(age.at.test) + posture + orientation + (1|subid), -->
<!--        data = d, -->
<!--        family = "binomial") -->

<!-- fullModelminsInt_Wrist_OP <- glmer(wristOP ~ scale(age.at.test) + posture + orientation + (1|subid), -->
<!--        data = d, -->
<!--        family = "binomial") -->
<!-- ``` -->
