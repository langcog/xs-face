...
## First try a normal logistic regressino, grouping by subject / posture / orientation
# wrangle data
dModelGLM <- d %>%
  group_by(subid, posture, orientation) %>%
  summarize(countDetected=sum(faceMT==TRUE), countTotal = sum(faceMT==FALSE), age.at.test=age.at.test[1])
y=cbind(dModelGLM$countDetected, dModelGLM$countTotal) # successes / failures for modeling proportions
dModelGLM$age.at.test=scale(dModelGLM$age.at.test) # scale and center continous predictor
# run model
normalGLM<-glm(y ~ dModelGLM$age.at.test + dModelGLM$posture + dModelGLM$orientation,family=binomial(logit))
# output results
normalGLM <- normalGLM %>%
  xtable %>%
  print(type="latex", comment = F, table.placement = "H") 

# Then fit a mixed effet model
fullModel <- glmer(faceMT ~ scale(age.at.test)+(posture)+(orientation) + (1|subid),
       data = d,
       family = "binomial")
...
fullModelminsInt_Faces_MTCNN <- glmer(faceNum_MT ~ scale(age.at.test) + posture + orientation + (1|subid),
 ...
 <- glm(faceNum_MT ~ scale(age.at.test) + posture + orientation,
       data = d,
       family = "binomial")
