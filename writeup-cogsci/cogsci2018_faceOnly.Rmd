...
## evaluate wrist detectors
performanceWrists <- ground_truth_wrists  %>%
  mutate(frame=as.numeric(frame), subid=video) %>%
  left_join(dlong)   %>%
  filter(detectorType=="wristOP")
...
fullModelminsInt_Faces_OP <- glm(faceNum_OP ~ scale(age.at.test) + posture + orientation,
       data = d,
       family = "binomial")


## try plain logistic regression
## think about classification techniques -- ?
