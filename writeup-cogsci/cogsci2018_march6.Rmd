...
These motor improvements lead to developmental cascades that impact children’s emerging social, cognitive, and linguistic abilities in various ways. For example, postural changes impact how children interact with their mothers: walking (vs. crawling) infants make different kinds of object-related bids for attention from their mothers and tend to hear more action directed statements (e.g., “open it”) [@karasik2014]. Infants also increase interactive behaviors in general and explore their surroundings more [@thurman2017spatial]. Further, in an observational study, @walle2014 found that children who were able to walk had both higher receptive and productive vocabularies. Thus, children’s ability to independently locomote may change their ability to access social information (e.g., faces, gaze) and in turn accelerate their learning.
...
While it is uncertain how much these changes are driven by the concurrent motoric changes described above versus by other developmental factors, recent work has found that whether an infant is crawling, sitting, or standing predicts how often they gaze at faces and bodies [@franchak2017see]. 

Here, we directly examine whether postural and locomotor developments change the availability of social information -- the presence of faces and hands. To do so, we recorded the visual experience of a group of infants in three age ranges (8, 12, and 16 months) using head-mounted cameras during a brief laboratory free-play session; children’s posture and orientation relative to their caregiver were also recorded from a third-person perspective and hand-annotated. We then capitalized on recent improvements in face and pose detection algorithms [@zhang2016; @cao2017realtime] to analyze the frequencies of faces and hands (using wrists as a proxy for the latter) in the child’s visual environment, both overall and relative to naming events by their caregivers. Only one piece of work to date has directly examined the impact of postural development to social information access in infants: a study by @franchak2017see found support for the motoric hypothesis presented above using a similar free-play experimental paradigm with head-mounted cameras on infants. This study required manual coding of the first 5 minutes of 15 minute videos from a cohort of 12-month olds; here we replicate and extend their findings using novel computer vision techniques to allow for video analysis at scale. We also analyze infants from a larger developmental age range (8, 12, and 16 month-olds).
...
# initial submission data - in case you want to reproduce.
...
dGLM_faces <- d %>%
  ...
  mutate(propFaces = countFaces / (countFaces + countNotFaces)) 

dGLM_wrists <- d %>%
  group_by(subid, posture, orientation) %>%
  summarize(countWrists=sum(wristOP==TRUE), countNotWrists = sum(wristOP==FALSE),     age.at.test=age.at.test[1])  %>%
  mutate(propWrists = countWrists / (countWrists + countNotWrists)) 

## run models
faceGLM <- glmer(cbind(countFaces,countNotFaces) ~ orientation + posture + scale(age.at.test) + 
               (orientation | subid), 
...
## output
cleanedNames_NoInt = c("Intercept","Close","Far","Sit","Stand","Age") # march 6th - prone is dropped from GLMs
# cleanedNames_NoInt = c("Intercept","Close","Far","Prone","Sit","Stand","Age") # from initial submission
...
```{r detByPosOrient, fig.env="figure*", fig.pos = "h", fig.align = "center", fig.width=6, fig.height=3, fig.cap = "Proportion of face and wrist detections in each age group as a function of children's posture (top panel) and orientation (bottom panel)."}
