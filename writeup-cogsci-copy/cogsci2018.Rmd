...
Next, we examined the impact of these changes

```{r fig.env="figure*", fig.pos = "h", fig.align = "center", fig.width=7, fig.height=3, fig.cap = "Proportion face (left) and hand (right) detections as a function of participant's age. Note that the axes are different; hands were detected overall much less frequently in this dataset." }
...
```{r faceByPostureFig, fig.env="figure*", fig.pos = "h", fig.align = "center", num.cols.cap=2, fig.width=7, fig.height=3, fig.cap = "Proportion of face detections as a function of infant's orientation, using the MTCNN3 face detections and OpenPose face detections (left two panels) and the proportion of hand detections as a function of infant's orientation, using OpenPose hand detections and the OpenPose wrist detections."}
...
ggplot(detectionsByPosture, aes(x = factor(age.grp), y = face, col = posture)) + 
  geom_boxplot() + 
  ylab("Proportion  Detections") + 
  xlab("Age (months)") + 
...
ggplot(detectionsByPosture, aes(x = factor(age.grp), y = detections, col = orientation)) + 
  geom_boxplot() + 
  ylab("Proportion  Detections") + 
  xlab("Age (months)") + 
  facet_grid(~detectorType) +
...
```{r xtable, results="asis"}
tab2 <- xtable::xtable(summary(fullModelminsInt_Hands)$coef, digits=3,
