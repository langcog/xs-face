...
levels(detectorType) <- c("MTCNN", "OpenPose") 
```
...
```{r fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=6, fig.height=3, fig.cap = "R plot" }
...
```{r fig.env="figure*", fig.pos = "h", fig.align = "center", fig.width=6, fig.height=2, fig.cap = "Proportion face (left) and hand (right) detections as a function of participant's age. Note that the axes are different; hands were detected overall much less frequently in this dataset." }
facebyAge <- dlong %>%
...
handsbyAge <- d %>%
  ...
  summarise(hands = mean(handOP)) %>%
  left_join(inc_subs) %>%
  mutate(len = as.numeric(len.min))

p1<-ggplot(facebyAge, aes(x = age.at.test, y = face, color = detectorType, size = len)) +
...
p2<-ggplot(handsbyAge, aes(x = age.at.test, y = hands, size = len)) +
    geom_jitter(width = .25) + 
    geom_smooth() +
    scale_x_continuous(breaks=c(8,12,16)) +
    scale_size_continuous(range=c(1,4),name = "Video Length (mins)") + 
    ylab("Proportion Hand Detections") + 
    xlab("Age (months)") + 
    theme_bw() +
    theme(legend.position="bottom", legend.box = "horizontal")
...
handbyPosture <- d %>%
  ...
  summarise(hand = mean(handOP)) %>%
  group_by(age.grp)

p2=ggplot(handbyPosture, aes(x = factor(age.grp), y = hand, col = posture)) + 
  geom_boxplot() + 
  ylab("Proportion Hand Detections") + 
  xlab("Age (months)") + 
...
p1=ggplot(facebyPosture, aes(x = factor(age.grp), y = face, col = posture)) + 
  geom_boxplot() + 
  ylab("Proportion Face Detections") + 
  xlab("Age (months)") + 
