...
author: "Mike Frank, continued Bria Long"
...
```{r}
d$face_num<-as.numeric(d$is_face=="TRUE")

mss <- d %>%
  group_by(age.grp, subid) %>%
  filter(!is.na(face_num)) %>% # filter out NA values
  summarise(face = mean(face_num)) %>%
  group_by(age.grp)
...
d <- d %>%
  group_by(subid) %>%
  filter(!is.na(face_num)) %>%
  mutate(face_avg = zoo::rollapply(data = face_num, 
...
```{r}
mss <- d %>%
  filter(!is.na(posture) & !is.na(orientation) & !is.na(face_num)) %>%
